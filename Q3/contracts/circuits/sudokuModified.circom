...
template sudoku() {
    ...
    component puzzle_range[9][9];
    component solution_range[9][9];
    ...
    for (var i=0; i<9; i++) {
        for (var j=0; j<9; j++) {
            ...
            puzzle_range[i][j] = RangeProof(4);
            solution_range[i][j] = RangeProof(4);

            puzzle_range[i][j].range[0] <== 0;
            puzzle_range[i][j].range[1] <== 9;
            puzzle_range[i][j].in <== puzzle[i][j];

            solution_range[i][j].range[0] <== 0;
            solution_range[i][j].range[1] <== 9;
            solution_range[i][j].in <== solution[i][j];

            1 === puzzle_range[i][j].out;
            1 === solution_range[i][j].out;
